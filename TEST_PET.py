...
def test_post_add_new_pet_with_all_valid_data(
        name='Глазастый',
        animal_type='Трехцветка дворовая',
        age='3',
        pet_photo_path='images/meow_01.jpg'
):
    """Добавление питомца meow_01 (c фото, все поля заполнены корректно):
...
def test_post_add_new_pet_with_vacant_data_with_foto(
    ...
    animal_type='',
    age='',
    pet_photo_path='images/meow_02.jpg'
):
    """Добавление питомца meow_02 (с фото, поля не заполнены):
    Выполняем POST-запрос на /api/pets.
...
def test_put_update_pet_with_not_valid_data_without_foto(
    ...
    """Изменение данных питомца meow_02 (все поля заполнены некорректно):
...
def test_delete_last_pet():
    """Удаление добавленного питомца meow_02:
    ...
    if not my_pets['pets']:
        pf.add_new_pet(auth_key, 'Cute', 'Lucky', "2", "images/hound_02.jpg")
        _, my_pets = pf.get_list_of_pets(auth_key, pf.MY_PETS)
...
def test_post_add_new_pet_with_not_valid_data_without_foto(
    ...
    """Добавление питомца hound_02 (без фото, все поля заполнены не корректно):
...
def test_post_update_pet_foto(
    pet_id='',
    pet_photo_path='images/hound_01.jpg'
):
    """Изменение данных питомца hound_01 (добавление фото):
